"""
render/stairs_renderer.py
Bakes actual vertex data per chunk for stair blocks (unlike doors/shadows
which remap a shared unit mesh via uniforms - stairs have a variable
number of boxes per shape, so baking is simpler). Rebuilt whenever the
chunk's mesh is rebuilt (see main.py), using the same collision boxes as
physics (world/stairs.py collision_boxes) so the visual shape always
matches what the player collides with.
"""
...
from world.stairs import collision_boxes
...
class StairsRenderer:
    def __init__(self, texture_atlas):
        ...
        self.shader = Shader(VERTEX_SHADER_SRC, FRAGMENT_SHADER_SRC, name="stairs")
        self.chunk_meshes: dict[tuple[int, int], tuple] = {}  # key -> (vao, vbo_pos, vbo_norm, vbo_uv, ebo, index_count)

    def _uv_for(self, block_id):
        rect = self.texture_atlas.uv_for(block_id, "all")
        if rect is None:
    ...
    def _build_chunk_geometry(self, instances):
        ...
        for (wx, wy, wz, block_id, facing, is_top, shape) in instances:
            u0, v0, u1, v1 = self._uv_for(block_id)
            for box in collision_boxes(facing, is_top, shape):
                bx0, by0, bz0, bx1, by1, bz1 = box
                for face in _BOX_FACES:
                    base = len(positions) // 1
    ...
    def rebuild_chunk(self, chunk, world):
        ...
        instances = chunk.build_stair_instances()
